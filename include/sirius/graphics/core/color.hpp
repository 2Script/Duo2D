...
template<std::size_t Channels, std::size_t BPC> requires (sizeof(typename acma::basic_color<Channels, BPC>::value_type) <= sizeof(std::size_t) && acma::basic_color<Channels, BPC>::perfect_sizing)
struct std::hash<acma::basic_color<Channels, BPC>> {
    constexpr std::size_t operator()(acma::basic_color<Channels, BPC> const& color) const noexcept {
        return static_cast<std::size_t>(std::bit_cast<typename acma::basic_color<Channels, BPC>::value_type>(color));
    }
};
