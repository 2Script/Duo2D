...
namespace d2d {
    struct descriptor_pool : vulkan_ptr<VkDescriptorPool, vkDestroyDescriptorPool> {
        ...
        static result<descriptor_pool> create(logical_device& device, std::span<VkDescriptorPoolSize, N> pool_sizes, std::uint32_t max_sets, std::bitset<N> enabled_bindings) noexcept;
    };
}
