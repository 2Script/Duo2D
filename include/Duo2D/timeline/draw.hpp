...
namespace d2d::timeline {
	...
	struct setup<draw<T>> {
		...
		result<vk::pipeline<vk::bind_point::graphics, T, N, Resources>> operator()(render_process<N, Resources, CommandGroupCount> const& proc) const noexcept {
			return make<vk::pipeline<vk::bind_point::graphics, T, N, Resources>>(proc.logical_device_ptr(), std::span<const VkFormat, 1>{&proc.swap_chain().format().pixel_format.id, 1}, proc.depth_image().format());
		};
	};
}
...
namespace d2d::timeline {
	...
	struct command<draw<T>> {
		...
		constexpr result<void> operator()(render_process<N, Resources, CommandGroupCount> const& proc, timeline::state<N, Resources, CommandGroupCount>&, vk::pipeline<vk::bind_point::graphics, T, N, Resources>& pipeline, sl::index_constant_type<CommandGroupIdx>) const noexcept {
