...
namespace d2d::impl {
	...
	using draw_command_buffer_filtered_index_sequence = sl::filtered_sequence_t<sl::index_sequence_of_length_type<N>, []<sl::index_t I>(sl::index_constant_type<I>){
	    return sl::get<sl::second_constant>(*std::next(BufferConfigs.begin(), I)).usage == usage_policy::draw_commands;
	}>; 
}
...
namespace d2d::timeline {
	template<sl::size_t N, buffer_config_table<N> BufferConfigs, sl::size_t CommandGroupCount>
	struct state {
		//using draw_buffer_offsets_type = sl::lookup_table<impl::draw_command_buffer_filtered_index_sequence<N, BufferConfigs, CommandGroupCount>::size(), buffer_key_t, sl::size_t>;
	public:
